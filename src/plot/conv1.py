...
def main(argv):
	...
	for opt, arg in opts:
		if opt == '-h':
			...
			ofile = arg
	# i.e. run ./plot/conv1sh.py -i plot/inp.json -o inc.json from src
	...
	for c, curve in enumerate([0.0, c2]):
		...
		plt.tight_layout()
		plt.savefig("%s%s%s" % ('plot/convergence_p',curve,'.pdf'))
	...
	for c, curve in enumerate([0.0, c2]):
		...
		plt.tight_layout()
		plt.savefig("%s%s%s" % ('plot/convergence_h',curve,'.pdf'))
